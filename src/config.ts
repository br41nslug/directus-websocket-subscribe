...
export async function getConfig(
  ...
  let config: DirectusWebsocketConfig = await emitter.emitFilter(
    "websocket.config",
    defaultWebsocketConfig
  );
  logger.debug(
    "[ WS ] added extension config overrides - " + JSON.stringify(config)
  );
  ...
  if ("AUTO_SUBSCRIPTION_COLLECTIONS" in env)
    config.autoSubscriptionCollections = env.AUTO_SUBSCRIPTION_COLLECTIONS;
