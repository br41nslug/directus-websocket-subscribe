...
export class DirectusWebsocketServer {
  ...
  public autoSubscriber: Function;
  ...
  public setAutoSubscriber(autoSubscriber: Function) {
  ...
  public hookServer(app: any, server: any) {
    ...
    logger.info(
      `Websocket listening on ws://localhost:${env.PORT}${this.config.path}`
    );
    server.on("upgrade", async (request: any, socket: any, head: any) => {
      ...
      if (
        ...
        (!this.config.public && !request.accountability.role)
      ) {
      ...
      logger.info(
        `request upgraded for user "${request.accountability.user || "public"}"`
      );
